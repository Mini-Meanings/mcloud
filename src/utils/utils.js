...
export const checkLogin = (router,params = {}) => {
  ...
  if(auth){
    if(router){
      ...
      const jsessionid = VueCookie.get("JSESSIONID");
      if(!jsessionid){
        ls.addItem('historyUrl',JSON.stringify({ historyUrl: window.location.href,account:"" }));//记录访问的地址
        window.location.href = '/#/login';
      }
 ...
 * @param menuList 是否显示
 */
// export const allVisibleRouterFilter=(router,menuList)=>{
//   debugger
//   let menu_router=[];
//   _.forEach(router,item=>{
//     let existRouter=menu_router.filter(item=>item.path==item.path);
//     if(item.meta&&item.meta.allVisible&&existRouter.length<=0){
//       const menu_router=item.meta.allVisible;
//       if(menu_router.length=== 1 &&menu_router[0]=== "*"){
//         menu_router.push(item);
//       }else{
//         _.forEach(menuList,allVisible=>{
//           if(filterArrValue(menu_router,allVisible)){
//             menu_router.push(item);
//             return;
//           }
//         });
