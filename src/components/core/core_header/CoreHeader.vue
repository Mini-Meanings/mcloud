  ...
  <section class="container-header">
    ...
    <section class="nav hidden-xs" v-if="switchProjectShow">
      <el-dropdown v-for="moduler in menuList" :class="{'head-nav': true}" menu-align="star" @command="navigate">
        <section class="header-name" v-if="moduler.link !==''"><a :href="moduler.link">{{moduler.name}}</a></section>
        <section class="header-name" v-else>{{moduler.name}}</section>
    ...
    <section class="other">
      <section class="project-wrapper hidden-xs hidden-sm"
               v-if="switchProjectShow&&!!simulation.length&&!!simulation[0].projectId">
      ...
      <section class="setting">
        <a class="header-icon" title="帮助文档" href="/module/management.html#/document" target="__blank"><i
...
//        else if (window.location.href.indexOf('/management.html#/message')+1) {
       ...
       */
      getMenuList() {
        this.$store.dispatch('getMenuList').then((val) => {
          console.log(val);
        })
