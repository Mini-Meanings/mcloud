      ...
      <div class="comment">
        <div style="display: block;padding-right: 10px;">
          <p class="comment-words">
            <!--<span class="comment-span">{{item.markMsg}}</span>-->
            {{item.markMsg}}
          </p>
...
//      formatSymbol() {
//        var span = document.getElementsByClassName('comment-span')[0];
//        var p = document.getElementsByClassName('comment-words')[0];
//        var spanWidth = span.offsetWidth;   // span的总共的宽度
//        var pWidth = p.offsetWidth;      // p的总共宽度
//        var forIndex = Math.floor(spanWidth / pWidth);
//        console.log(forIndex)
//        var spanText = span.innerHTML;                           // span里面的字
//        var spanTextLength = span.innerHTML.length;   // span里面字符串的长度
//        var spanLetterWidth = spanWidth / spanTextLength;  // 每个字符所占的宽度
//        var spanEndInPIndex = Math.floor(pWidth / spanLetterWidth);  // 每一行的末尾的 span中字符串的索引
//        var spanTextArr = spanText.split('');
//        for (var i = 1; i <= forIndex; i++) {
//          spanTextArr.splice(spanEndInPIndex * i, 1, spanTextArr[spanEndInPIndex * i] + ' ');
//        }
//        var spanStr = spanTextArr.join('');
//        console.log(spanStr);
