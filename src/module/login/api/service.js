...
export const requestLogin = params => {
  ...
  return axios.post(api.login, params ).then(res => {
    ...
    if(errorCode === "600000"){
      VueCookie.set("JSESSIONID",result.jsession,{ domain: domain});
      VueCookie.set("OAM_ID",result.oamid,{ domain: domain});
      VueCookie.set("mideatest_sso_token",result.mipssotoken,{ domain: domain});
      let user={
        email:result.email,
